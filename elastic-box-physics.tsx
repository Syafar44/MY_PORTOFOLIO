...
export default function ElasticBoxPhysics({ config = {} }: ElasticBoxPhysicsProps) {
  ...
  const handlePointerDown = (e: { preventDefault: () => void; clientX: number; clientY: number }) => {
    if (isMobile) return // Skip for mobile
  ...
  return (
    <div
        ref={containerRef}
        className="relative w-full h-[500px] touch-none overflow-visible"
      > 
      {/* Content component for text and social sharing */}
      {/* <Conten ent isMobile={isMobile} config={mergedConfig} /> */}
